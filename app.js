...
async function connectSharedSync(){
  ...
  catch(e){updateSyncUI("Connection failed: "+e.message);toast("Could not connect")}
}
async function migrateExistingData(){
  if(localStorage.getItem("kt_migration_complete")==="yes"){await pullSharedData(true);return}
  exportBackup(true);
  const result=await apiRequest("migrate",{data:syncPayload()});
...
function exportBackup(silent=false){
  ...
  const blob=new Blob([JSON.stringify(backup,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`king-tech-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);if(!silent)toast("Backup exported")
}
