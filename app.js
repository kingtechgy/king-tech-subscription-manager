...
let balanceEntries = JSON.parse(localStorage.getItem("kt_balance_entries")||"[]");
...
function dateOnly(value){const normalized=dateInputValue(value);if(!normalized)return null;const date=new Date(normalized+'T00:00:00');return Number.isNaN(date.getTime())?null:date}
...
function saveLocalOnly(){
  ...
  localStorage.setItem("kt_balance_entries",JSON.stringify(balanceEntries));
...
function periodLabel(){return financialPeriod==="week"?"This week":financialPeriod==="all"?"All time":"This month"}
function filteredPayments(){return payments.filter(p=>dateWithinFinancialPeriod(p.date||p.createdAt))}
function filteredExpenses(){return expenses.filter(x=>dateWithinFinancialPeriod(x.date||x.createdAt))}
function filteredBalanceEntries(){return balanceEntries.filter(x=>dateWithinFinancialPeriod(x.date||x.createdAt))}
function setFinancialPeriod(value){financialPeriod=["week","month","all"].includes(value)?value:"month";localStorage.setItem("kt_financial_period",financialPeriod);["paymentPeriodFilter","reportPeriodFilter"].forEach(id=>{const el=document.getElementById(id);if(el)el.value=financialPeriod});renderPayments();renderReports()}
...
function render(){refreshAutomaticStatuses();
 const active=customers.filter(c=>c.status==="Active").length,due=customers.filter(c=>["Due Soon","Due Today"].includes(c.status)).length,over=customers.filter(c=>["Overdue","Expired"].includes(c.status)).length,slots=availableInventoryCount();
...
function renderCustomers(){
 ...
 document.getElementById("customerRows").innerHTML=rows.map(c=>{const cid=JSON.stringify(String(c.id)),credentials=c.login||c.password?`<div class="muted" style="margin-top:5px">Login: ${c.login||"Not added"}${c.password?` • Password saved`:""}</div>`:"";return `<tr><td><strong>#${c.billNumber}</strong></td><td><div class="name">${c.name}</div><div class="muted">${c.phone||"Phone not added"}${c.email?` • ${c.email}`:""}${c.referenceNumber?` • Ref ${c.referenceNumber}`:""}</div></td><td>${c.service}<div class="muted">${c.plan}${c.signupFee?` • Sign-up fee ${money(c.signupFee)}`:""}</div></td><td>${c.account||"—"}<div class="muted">${c.profile||""}</div>${credentials}</td><td>${money(c.amount)}</td><td>${c.due}</td><td>${c.expiry}</td><td><span class="badge ${slug(c.status)}">${c.status}</span></td><td><button class="icon-btn" onclick='editCustomer(${cid})'>Edit</button> <button class="icon-btn" onclick='markPaid(${cid})'>Paid</button> <button class="icon-btn" onclick='whatsApp(customerById(${cid}))'>Message</button></td></tr>`}).join("")||`<tr><td colspan="9">No matching customers.</td></tr>`;
}
...
function renderReports(){const periodPayments=filteredPayments(),periodExpenses=filteredExpenses(),periodEntries=filteredBalanceEntries(),monthlyIncome=billableCustomers().reduce((s,c)=>s+Number(c.amount||0),0),received=periodPayments.reduce((s,p)=>s+Number(p.amount||0),0),exp=periodExpenses.reduce((s,x)=>s+Number(x.amount||0),0),moneyIn=periodEntries.filter(x=>x.direction==="in").reduce((s,x)=>s+Number(x.amount||0),0),moneyOut=periodEntries.filter(x=>x.direction==="out").reduce((s,x)=>s+Number(x.amount||0),0),cashProfit=received+moneyIn-exp-moneyOut;document.getElementById("reportStats").innerHTML=[
 [money(monthlyIncome),"Monthly Income","Current billable subscriptions","customers"],
 [money(received),"Payments Received",periodLabel(),"payments"],
 [money(exp),"Expenses",periodLabel(),"expenses"],
 [money(cashProfit),"Current Balance",`${periodLabel()} after all money in and out`,"reports"]
].map(x=>`<div class="stat clickable-card" onclick="openSection('${x[3]}')"><span>${x[1]}</span><strong>${x[0]}</strong><small>${x[2]}</small></div>`).join("");const balanceSummary=document.getElementById("balanceSummary");if(balanceSummary)balanceSummary.innerHTML=[[money(received),"Customer Payments"],[money(moneyIn),"Other Money In"],[money(exp+moneyOut),"Total Money Out"],[money(cashProfit),"Balanced Cash"]].map(x=>`<div class="stat"><span>${x[1]}</span><strong>${x[0]}</strong><small>${periodLabel()}</small></div>`).join("");const entryRows=document.getElementById("balanceEntryRows");if(entryRows)entryRows.innerHTML=[...periodEntries].sort((a,b)=>String(b.date||b.createdAt||"").localeCompare(String(a.date||a.createdAt||""))).map(x=>{const xid=JSON.stringify(String(x.id));return `<tr><td>${x.date||""}</td><td>${x.category||"Adjustment"}</td><td>${x.description||"—"}</td><td><span class="badge ${x.direction==="in"?"active":"overdue"}">${x.direction==="in"?"Money In":"Money Out"}</span></td><td>${x.direction==="out"?"−":"+"}${money(x.amount)}</td><td>${x.agent||"—"}</td><td><button class="icon-btn" onclick='removeBalanceEntry(${xid})'>Remove</button></td></tr>`}).join("")||`<tr><td colspan="7">No balance entries for ${periodLabel().toLowerCase()}.</td></tr>`;const services=[...new Set(billableCustomers().map(c=>c.service).filter(Boolean))];document.getElementById("profitByService").innerHTML=services.map(service=>{const monthly=billableCustomers().filter(c=>c.service===service).reduce((sum,c)=>sum+Number(c.amount||0),0),paid=periodPayments.filter(p=>paymentService(p)===service).reduce((sum,p)=>sum+Number(p.amount||0),0);return `<div class="notice"><div><strong>${service}</strong><small>${money(paid)} received • ${periodLabel()}</small></div><span>${money(monthly)} monthly</span></div>`}).join("")||'<div class="notice"><strong>No subscription income recorded yet</strong></div>';const serviceCounts=customers.reduce((m,c)=>(m[c.service]=(m[c.service]||0)+1,m),{}),topService=Object.entries(serviceCounts).sort((a,b)=>b[1]-a[1])[0]?.[0]||"—",upcomingProviderPayments=accounts.filter(a=>{const days=daysFromToday(a.providerDue);return days!==null&&days>=0&&days<=30}).length;document.getElementById("snapshot").innerHTML=`<div class="notice"><strong>Most customers</strong><span>${topService}</span></div><div class="notice"><strong>Available accounts / slots</strong><span>${availableInventoryCount()}</span></div><div class="notice"><strong>Provider payments in next 30 days</strong><span>${upcomingProviderPayments}</span></div><div class="notice"><strong>Expected monthly income</strong><span>${money(monthlyIncome)}</span></div>`}
...
function editCustomer(id){
 ...
 ["name","phone","email","referenceNumber","billNumber","service","plan","amount","signupFee","account","login","password","status","due","expiry"].forEach(k=>{if(form.elements[k])form.elements[k].value=["due","expiry"].includes(k)?dateInputValue(c[k]):(c[k]??"")});
 updateSubscriptionPlans();
...
function toggleCustomerPassword(){const input=document.getElementById("customerPassword");if(input)input.type=input.type==="password"?"text":"password"}
...
function removeBalanceEntry(id){if(!confirm("Remove this balance entry?"))return;balanceEntries=balanceEntries.filter(x=>String(x.id)!==String(id));save();toast("Balance entry removed")}
const balanceEntryForm=document.getElementById("balanceEntryForm");if(balanceEntryForm){balanceEntryForm.elements.date.value=new Date().toISOString().slice(0,10);balanceEntryForm.onsubmit=e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.target));balanceEntries.unshift({...f,id:Date.now(),amount:Number(f.amount||0),agent:getAgentName(),createdAt:new Date().toISOString()});e.target.reset();e.target.elements.date.value=new Date().toISOString().slice(0,10);e.target.closest(".modal").classList.remove("open");save();toast("Balance entry saved")}}
...
ensureBillNumbers();
fillSubscriptionServices();
["paymentPeriodFilter","reportPeriodFilter"].forEach(id=>{const el=document.getElementById(id);if(el){el.value=financialPeriod;el.onchange=e=>setFinancialPeriod(e.target.value)}});
render();
...
window.applyFirebaseSnapshot=data=>{
  ...
  customers=data.customers;accounts=data.accounts||[];payments=data.payments||[];expenses=data.expenses||[];deletedAccounts=data.deletedAccounts||[];providerPayments=data.providerPayments||[];balanceEntries=data.balanceEntries||[];
...
function syncPayload(){
  ...
  return {schemaVersion:"3.5",sheetId:LIVE_SHEET_ID,deviceId:deviceId(),agent:getAgentName(),updatedAt:new Date().toISOString(),customers,accounts,payments,expenses,deletedAccounts,providerPayments,balanceEntries,reminderHistory:Object.entries(bulkReminderLog||{}).map(([key,sentAt])=>({id:key,key,sentAt}))};
}
...
async function joinSharedDatabase(){
  ...
  try{
    ...
    customers=d.customers||[];accounts=d.accounts||[];payments=d.payments||[];expenses=d.expenses||[];deletedAccounts=d.deletedAccounts||[];providerPayments=d.providerPayments||[];balanceEntries=d.balanceEntries||[];
...
async function pullSharedData(showToast=false){
  ...
  try{
    ...
    if(Array.isArray(d.deletedAccounts))deletedAccounts=d.deletedAccounts;
    if(Array.isArray(d.providerPayments))providerPayments=d.providerPayments;
    if(Array.isArray(d.balanceEntries))balanceEntries=d.balanceEntries;
    if(d.reminderLog&&typeof d.reminderLog==="object")bulkReminderLog=d.reminderLog;
